...
@endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 portfolio-grid">
                    @foreach ($portfolios as $idx => $item)
                        @php
                            $itemType = $item['type'] ?? 'image';
                            $itemTitle = $item['title'] ?? ($itemType == 'video' ? 'Video Project' : 'Showcase');
                            $itemDesc = $item['description'] ?? '';
                            $typeColors = [
                                'image' => 'primary',
                                'video' => 'red-500',
                                'game' => 'cyan-400',
                                'website' => 'orange-400',
                                'mobile_app' => 'emerald-400',
                            ];
                            $typeIcons = [
                                'image' => 'ti-photo',
                                'video' => 'ti-player-play-filled',
                                'game' => 'ti-device-gamepad-2',
                                'website' => 'ti-world',
                                'mobile_app' => 'ti-device-mobile',
                            ];
                            $accentColor = $typeColors[$itemType] ?? 'primary';
                            $accentIcon = $typeIcons[$itemType] ?? 'ti-photo';
                        @endphp

                        @if($itemType === 'mobile_app')
                            {{-- Mobile App — Phone Simulator Card --}}
                            <div class="portfolio-item" data-category="{{ Str::slug($item['category']) }}">
                                <div class="bg-animazon-navy border border-animazon-border/50 rounded-2xl p-6 flex flex-col items-center transition-all duration-500 hover:border-emerald-500/50 hover:shadow-[0_0_40px_rgba(16,185,129,0.1)]">
                                    {{-- Category + Title --}}
                                    <div class="text-center mb-4 w-full">
                                        <span class="text-xs font-bold text-emerald-400 tracking-widest uppercase block mb-1">{{ $item['category'] }}</span>
                                        <h4 class="text-lg font-bold text-white">{{ $itemTitle }}</h4>
                                        @if($itemDesc)
                                            <p class="text-animazon-muted text-sm mt-1">{{ Str::limit($itemDesc, 60) }}</p>
                                        @endif
                                    </div>
                                    ...
                                    <div class="phone-simulator-frame">
                                        ...
                                        <div class="phone-screen">
                                            @if(!empty($item['mobile_screenshots']))
                                                <div class="phone-carousel" data-phone-id="phone-{{ $idx }}">
                                                    @foreach($item['mobile_screenshots'] as $ssIdx => $screenshot)
                                                        <img src="{{ \App\Models\Utility::get_file('uploads/landing_page_image/' . $screenshot) }}"
                                                             class="phone-slide {{ $ssIdx === 0 ? 'active' : '' }}"
                                                             alt="App Screen {{ $ssIdx + 1 }}" loading="lazy">
                                                    @endforeach
                                                </div>
                                                @if(count($item['mobile_screenshots']) > 1)
                                                    <div class="phone-nav">
                                            ...
                                            @elseif(!empty($item['image']))
                                                <img src="{{ \App\Models\Utility::get_file('uploads/landing_page_image/' . $item['image']) }}"
                                                     class="w-full h-full object-cover" alt="{{ $itemTitle }}" loading="lazy">
                                            @else
                                                <div class="w-full h-full flex items-center justify-center bg-animazon-black text-animazon-muted">
                                                    <i class="ti ti-device-mobile text-4xl"></i>
                                                </div>
                        ...
                        @elseif($itemType === 'game')
                            ...
                            <div class="portfolio-item" data-category="{{ Str::slug($item['category']) }}">
                                <div class="group bg-animazon-navy border border-animazon-border/50 rounded-2xl overflow-hidden transition-all duration-500 hover:border-cyan-500/50 hover:shadow-[0_0_40px_rgba(34,211,238,0.1)] hover:-translate-y-1">
                                    <div class="aspect-video relative overflow-hidden">
                                        @if(!empty($item['image']))
                                            <img src="{{ \App\Models\Utility::get_file('uploads/landing_page_image/' . $item['image']) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="{{ $itemTitle }}" loading="lazy">
                                        @else
                                            <div class="w-full h-full bg-gradient-to-br from-cyan-900/30 to-animazon-black flex items-center justify-center">
                                                <i class="ti ti-device-gamepad-2 text-5xl text-cyan-400/50"></i>
                                            </div>
                                    ...
                                    <div class="p-6">
                                        <span class="text-xs font-bold text-cyan-400 tracking-widest uppercase mb-1 block">{{ $item['category'] }}</span>
                                        <h4 class="text-lg font-bold text-white mb-1">{{ $itemTitle }}</h4>
                                        @if($itemDesc)
                                            <p class="text-animazon-muted text-sm">{{ Str::limit($itemDesc, 80) }}</p>
                                        @endif
                                    </div>
                            ...
                            <div class="portfolio-item" data-category="{{ Str::slug($item['category']) }}">
                                <div class="group bg-animazon-navy border border-animazon-border/50 rounded-2xl overflow-hidden transition-all duration-500 hover:border-orange-500/50 hover:shadow-[0_0_40px_rgba(249,115,22,0.1)] hover:-translate-y-1">
                                    <div class="aspect-video relative overflow-hidden">
                                        ...
                                        @if(!empty($item['website_url']))
                                            <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <a href="{{ $item['website_url'] }}" target="_blank" class="w-16 h-16 bg-orange-500 rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-110 transition-transform">
                                ...
                                <div class="group bg-animazon-navy border border-animazon-border/50 rounded-2xl overflow-hidden transition-all duration-500 hover:border-red-500/50 hover:shadow-[0_0_40px_rgba(239,68,68,0.1)] hover:-translate-y-1">
                                    <div class="aspect-video relative overflow-hidden">
                                        ...
                                        @if(!empty($item['image']))
                                            <img src="{{ \App\Models\Utility::get_file('uploads/landing_page_image/' . $item['image']) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="{{ $itemTitle }}" loading="lazy">
                                        @elseif($video_id)
                                            <img src="https://img.youtube.com/vi/{{ $video_id }}/maxresdefault.jpg" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="{{ $itemTitle }}" loading="lazy">
                                        @endif
                                        @if($video_id)
                                            <div class="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/10 transition-all">
                                                <div class="w-16 h-16 bg-red-500 text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform cursor-pointer" onclick="openVideoPlayer('{{ $video_id }}')">
                        ...
                        @else
                            {{-- Image Card (default) --}}
                            <div class="portfolio-item" data-category="{{ Str::slug($item['category']) }}">
                                <div class="group bg-animazon-navy border border-animazon-border/50 rounded-2xl overflow-hidden transition-all duration-500 hover:border-primary/50 hover:shadow-[0_0_40px_rgba(var(--primary-glow),0.15)] hover:-translate-y-1">
                                    <div class="aspect-[4/3] relative overflow-hidden">
                                        @if(!empty($item['image']))
                                            <img src="{{ \App\Models\Utility::get_file('uploads/landing_page_image/' . $item['image']) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="{{ $itemTitle }}" loading="lazy">
                                        @else
                                            <div class="w-full h-full bg-gradient-to-br from-primary/20 to-animazon-black flex items-center justify-center">
                                                <i class="ti ti-photo text-5xl text-primary/50"></i>
                                            </div>
                                        @endif
                                        <!-- Overlay -->
                                        <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/60 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                                            <span class="text-xs font-bold text-primary tracking-widest uppercase mb-1 block">{{ $item['category'] }}</span>
                                            <h4 class="text-lg font-bold text-white">{{ $itemTitle }}</h4>
                                            @if($itemDesc)
                                                <p class="text-animazon-muted text-sm mt-1">{{ Str::limit($itemDesc, 80) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        document.querySelectorAll('.portfolio-filter').forEach(button => {
            button.addEventListener('click', () => {
                document.querySelectorAll('.portfolio-filter').forEach(b => b.classList.remove('active', 'border-primary', 'text-white', 'bg-primary/20'));
                button.classList.add('active', 'border-primary', 'text-white', 'bg-primary/20');
    ...
    <style>
        .portfolio-filter.active { border-color: rgb(var(--color-primary)); color: white; background: rgba(var(--color-primary), 0.15); }
