...
// Route::post('import/vender', [VenderController::class, 'import'])->name('vender.import');
    ...
    // Currency Management
    Route::get('currencies', [CurrencyController::class, 'index'])->name('currencies.index')->middleware(['auth', 'XSS']);
    Route::post('currencies/switch', [CurrencyController::class, 'switch'])->name('currencies.switch')->middleware(['auth', 'XSS']);
    Route::get('currencies/current', [CurrencyController::class, 'getCurrent'])->name('currencies.current')->middleware(['auth', 'XSS']);
    Route::post('currencies/convert', [CurrencyController::class, 'convert'])->name('currencies.convert')->middleware(['auth', 'XSS']);
    // Admin Routes
    Route::get('admin/currencies/manage', [CurrencyController::class, 'manage'])->name('currencies.manage')->middleware(['auth', 'XSS']);
    Route::post('admin/currencies/{currency}/toggle', [CurrencyController::class, 'toggle'])->name('currencies.toggle')->middleware(['auth', 'XSS']);
    Route::post('admin/currencies/{currency}/default', [CurrencyController::class, 'setDefault'])->name('currencies.default')->middleware(['auth', 'XSS']);
    Route::post('admin/currencies/update-rates', [CurrencyController::class, 'updateRates'])->name('currencies.update-rates')->middleware(['auth', 'XSS']);
    // End Currency Management

    // Public API - Currencies (no auth required for dropdown)
    Route::get('api/currencies', [CurrencyController::class, 'listPublic'])->name('api.currencies.list');
