    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('manage system settings')) {
            ...
            if (
                ...
                $post['SITE_RTL'] = $request->has('SITE_RTL') ? 'on' : 'off';
                $post['display_landing_page'] = $request->has('display_landing_page') ? 'on' : 'off';
                $post['gdpr_cookie'] = $request->has('gdpr_cookie') ? 'on' : 'off';
                $post['enable_signup'] = $request->has('enable_signup') ? 'on' : 'off';
                $post['email_verification'] = $request->has('email_verification') ? 'on' : 'off';
                $post['cust_theme_bg'] = $request->has('cust_theme_bg') ? 'on' : 'off';
                $post['cust_darklayout'] = $request->has('cust_darklayout') ? 'on' : 'off';

                if (isset($request->color_flag) && $request->color_flag == 'false' && isset($request->color)) {
                    $post['color'] = $request->color;
                } elseif (isset($request->primary_color)) {
                    $post['color'] = 'custom-color';
